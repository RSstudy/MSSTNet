...
def plotAbundancesSimple(abundances, abundance_path, rmsesave):
    # 调整 abundances 的维度顺序（如果需要）
    abundances = np.transpose(abundances, axes=[1, 0, 2])
    num_endmembers = abundances.shape[2]  # 获取端元数量
    ...
    # 计算 abundance_index 和 MSE_abundance（假设 order_abundance 不需要 abundanceGT）
    abundance_index = np.arange(num_endmembers)  # 直接使用默认顺序
    MSE_abundance = np.zeros(num_endmembers)  # 假设 MSE 为 0（因为没有 GT）
    ...
    for i in range(num_endmembers):
        ...
        im = ax.imshow(abundances[:, :, abundance_index[i]], cmap=cmap)  # 绘制丰度图
        plt.colorbar(im, cax=cax, orientation='horizontal')  # 添加颜色条
        ax.set_title(format(MSE_abundance[i], '.3f'))  # 设置子图标题（MSE 值）
        ax.get_xaxis().set_visible(False)  # 隐藏坐标轴
        ax.get_yaxis().set_visible(False)
        rmsesave.append(MSE_abundance[i])  # 保存 MSE 值
    ...
    plt.tight_layout()
    plt.rcParams.update({'font.size': 15})
    plt.suptitle("RMSE: {:.3f}".format(MSE_abundance.mean()))  # 设置总标题
